-- =============================================================================
-- Nortis | Control de aplicaciones: regla de incidente
-- =============================================================================
-- Cuando el agente reporta un app_open de un proceso que la politica tiene en
-- apps.blocklist (con modo distinto de allow), se abre un incidente. La accion
-- base es 'terminate' (modo block) o 'alert'; si el agente ya termino el proceso,
-- el evento trae enforcement='terminated' y ese gana en el incidente.

-- Severidad: una aplicacion no autorizada corriendo es notable -> alta.
...
as $function$
  select case p_rule
    ...
    when 'apps.proceso_no_autorizado'             then 'high'
...
begin
  ...
  from (
    ...
    cross join lateral (
      select 'storage.carpeta_no_autorizada' as rule_key, 'storage' as channel, 'block' as action, e.payload ->> 'path' as sample
       where jsonb_array_length(coalesce(sp.config #> '{storage,allowed_paths}', '[]'::jsonb)) > 0
         ...
         and not exists (select 1 from jsonb_array_elements_text(sp.config #> '{storage,allowed_paths}') as ap(path)
            where starts_with(lower(e.payload ->> 'path'), lower(ap.path)))
      union all
      ...
      select 'classification.dato_vigilado', 'storage',
             coalesce(sp.config #>> '{classification,mode}', 'alert'),
             e.payload ->> 'path'
       where e.event_type in ('file_created', 'file_modified')
      ...
      union all
      -- Aplicacion no autorizada: app_open de un proceso de la lista de bloqueo.
      select 'apps.proceso_no_autorizado', 'apps',
             case when sp.config #>> '{apps,mode}' = 'block' then 'terminate' else 'alert' end,
             e.payload ->> 'app'
       where coalesce(sp.config #>> '{apps,mode}', 'allow') <> 'allow'
         and e.event_type = 'app_open'
         and lower(e.payload ->> 'app') in (
              select lower(value) from jsonb_array_elements_text(coalesce(sp.config #> '{apps,blocklist}', '[]'::jsonb)))
      union all
  ...
  on conflict (organization_id, endpoint_id, rule_triggered, occurrence_day) do update
    set event_snapshot = excluded.event_snapshot, event_id = excluded.event_id,
        detected_at = excluded.detected_at, enforcement_action = excluded.enforcement_action,
