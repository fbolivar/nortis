-- =============================================================================
-- Nortis | Politicas basadas en la clasificacion (2)
-- =============================================================================
-- La clasificacion pasa de observar a decidir: el perfil puede declarar clases
-- VIGILADAS y, cuando un archivo de esa clase se crea o modifica, se abre un
-- incidente aunque la carpeta y la extension estuvieran permitidas. Es la
-- promesa central de un DLP: se vigila el DATO, no la carpeta donde cae.
--
-- La regla vive en sp.config -> classification:
--   { "watched": ["Financiero", "Datos personales"], "mode": "alert" | "block" }
-- El agente corre en modo usuario, asi que sobre archivos el nivel real es
-- "solo registra" (ver APLICACION_POR_CANAL en la consola). 'block' se conserva
-- por contrato y la UI lo dice sin rodeos; hoy ambos modos abren incidente.
--
-- La severidad sale de dlp_rule_severity (else 'medium') y la escalada por clase
-- sensible de la migracion (1) la sube a 'high' sola cuando corresponde.
...
begin
  ...
  from (
    ...
    cross join lateral (
      ...
      union all
      -- (2) Clase vigilada: el archivo lleva una clasificacion que el perfil
      -- declaro en classification.watched. Independiente de carpeta/extension.
      select 'classification.dato_vigilado', 'storage',
             case when sp.config #>> '{classification,mode}' = 'block' then 'block' else 'alert' end,
             e.payload ->> 'path'
       where e.event_type in ('file_created', 'file_modified')
         and e.payload ? 'classification'
         and lower(e.payload ->> 'classification') in (
              select lower(value) from jsonb_array_elements_text(coalesce(sp.config #> '{classification,watched}', '[]'::jsonb)))
      union all
