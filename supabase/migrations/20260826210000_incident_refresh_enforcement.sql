-- =============================================================================
-- Nortis | El incidente refresca su enforcement_action al reagrupar
-- =============================================================================
-- Los incidentes se agrupan por (regla + dia), asi que varios eventos del mismo
-- tipo caen en la misma fila. El on-conflict de detect_dlp_incidents actualizaba
-- muestra, clasificacion y severidad, pero NO enforcement_action: si el primer
-- evento del dia fue 'alert' y uno posterior fue 'quarantine' (el agente retiro
-- el archivo), el incidente seguia mostrando 'alert'. Se anade enforcement_action
-- al update para que refleje la accion mas reciente del grupo (que ya se calcula
-- como el enforcement mas nuevo, o la accion base de la regla).
--
-- Unico cambio respecto a la version anterior: el SET del on-conflict.
