...
--
-- Un tipo de tarea nuevo (scan_av) que arranca un escaneo rapido o completo. El
-- resultado (fecha del escaneo + amenazas) llega por el inventario de postura,
-- no por la propia tarea.

alter type public.agent_task_kind add value if not exists 'scan_av';
...
begin
  ...
  if p_kind not in ('install_msi','push_file','restart','run_script','lock','wipe','screenshot','message','kill','uninstall','wake','schedule_script','scan_av') then
