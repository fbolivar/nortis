-- =============================================================================
-- Nortis | La accion de la clase vigilada refleja el modo (alert | quarantine)
-- =============================================================================
-- Antes la regla classification.dato_vigilado fijaba la accion en 'alert' salvo
-- el modo 'block' (que sobre archivos solo registraba). Ahora el modo util es
-- 'quarantine': el agente RETIRA el archivo a cuarentena y el evento llega con
-- enforcement='quarantine'. Se pasa el modo tal cual como accion base del grupo,
-- para que un incidente sin eventos de enforcement (agente viejo) muestre al
-- menos lo que la politica pidio, y uno con enforcement lo confirme.
--
-- Unico cambio respecto a la version anterior: la accion de esa rama.
...
begin
  ...
  from (
    ...
    cross join lateral (
      ...
      union all
      -- Clase vigilada. La accion base es el modo declarado (alert | quarantine);
      -- si el agente retiro el archivo, el evento trae enforcement='quarantine' y
      -- ese gana en el coalesce de arriba.
