-- =============================================================================
-- Nortis | Ejecucion remota: run_script (script firmado)
-- =============================================================================
-- Ejecutar un script (PowerShell/cmd) FIRMADO por la consola. Mismo canal firmado
-- que install_msi/push_file/restart: la firma Ed25519 sigue siendo la unica
-- puerta y emitir exige admin+MFA (+ sede, para un delegado). No es una clase de
-- riesgo nueva — install_msi ya permite ejecutar cualquier cosa —, solo la vuelve
-- directa.

alter type public.agent_task_kind add value if not exists 'run_script';

-- issue_agent_task: permitir 'run_script'. Conserva el chequeo de sede (Fase 3) y
-- el p_not_before (programador). El resto es identico.
...
begin
  ...
  if p_kind not in ('install_msi', 'push_file', 'restart', 'run_script') then
